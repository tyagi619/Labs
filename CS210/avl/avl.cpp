...
node* avl::rotateRight(node* j){
   ...
   */
	...
	node *left_child,*b;
	left_child=j->getLeft();
	b=left_child->getRight();
	j->setLeft(b);
	if(b!=NULL){
		b->setParent(j);
	}	
	left_child->setRight(j);
	j->setParent(left_child);
...
node* avl::rotateLeft(node* j){
	node *right_child,*b;
	right_child=j->getRight();
	b=right_child->getLeft();
	j->setRight(b);
	if(b!=NULL){
		b->setParent(j);
	}	
	right_child->setLeft(j);
	j->setParent(right_child);
